...
### Features
- **KISS/TCP as an optional per-node transport**: each node can be reached over its KISS/TCP interface (TCP port 8001, ESP32 firmware v1.4+) instead of ext-udp, selected per node in Settings. ext-udp stays the default. Over KISS the monitor gains the full APRS position comment, the digipeater path, the `/R=` relay-node list, the `/N` neighbour count and RSSI/SNR per frame. Sending works over KISS with a per-send delivery result from the node's `0xF0` frame. Optional HMAC-SHA256 auth (`--kiss auth on`) reuses the node password.

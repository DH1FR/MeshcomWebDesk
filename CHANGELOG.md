...
### Features
- **KISS/TCP as an optional per-node add-on** – each node can now *additionally* be connected over its **KISS/TCP** interface (TCP port 8001, ESP32 firmware v1.4+), on top of the always-on ext-udp base – enabled with a checkbox per node in Settings. Over KISS the monitor shows what ext-udp cannot: the **full APRS position comment**, the **digipeater path**, the **`/R=` relay-node list** and **`/N` neighbour count**, and **RSSI/SNR per frame**. Sending works over KISS too, with a per-send delivery result (accepted + `msg_id` / rejected + reason) read from the node's `0xF0` frame. Optional **HMAC-SHA256 authentication** (`--kiss auth on` on the node) reuses the node password, identical to the NET Console scheme.

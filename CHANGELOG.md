...
### Features
- **KISS/TCP as an optional per-node add-on** – each node can now *additionally* be connected over its **KISS/TCP** interface (TCP port 8001; requires a MeshCom firmware with the KISS/TCP interface), on top of the always-on ext-udp base – enabled with a checkbox per node in Settings. Over KISS the monitor shows what ext-udp cannot: the **full APRS position comment**, the **digipeater path**, the **`/R=` relay-node list** and **`/N` neighbour count**, and **RSSI/SNR per frame**. Sending works over KISS too, with a per-send delivery result (accepted + `msg_id` / rejected + reason) read from the node's `0xF0` frame. Optional **HMAC-SHA256 authentication** (`--kiss auth on` on the node) reuses the node password, identical to the NET Console scheme.
- **KISS hub** – WebDesk can hold the single KISS connection to a node and re-serve it as its own KISS/TCP listener, so **Direwolf, YAAC, APRSdroid** and other KISS clients connect *through* WebDesk instead of competing for the node's one connection slot. Configurable port, target node and localhost/LAN binding under Settings → 🔀 KISS Hub. Received RF frames fan out to all connected clients; a client's transmit is injected via the node and its `0xF0` result routed back to that client.
- **SrcInfo frame (KISS `0x20`)** – MeshCom SSIDs above 15 (`-16`…`-99`) do not fit the 4-bit AX.25 address field and are clamped to `-15` on the wire. When that happens the firmware now sends the full origin callsign in a separate frame; WebDesk uses it as the real sender for the monitor **and the reply addressee**, so a reply to a two-digit-SSID station is addressed correctly.
- **KISS and ext-udp shown as separate status indicators** – for a KISS-primary node the status bar shows a **KISS** and an **ext-udp** dot side by side. The paths are complementary: KISS carries foreign traffic, monitor detail and the TX result; ext-udp carries the node's own position, the telemetry panel and firmware/HW. A yellow ext-udp dot (with a hint) flags when the node's own data is not arriving.
- **Per-node password** – the node password field is now per node and feeds both the NET Console (HMAC) and, with `--kiss auth on`, KISS authentication. The unused "TLS certificate fingerprint" field was removed.
- **KISS console commands** – `--kiss`, `--kiss tx`, `--kiss meta` added to the Console Command Helper (Network group).

### Bugfixes
- **Bare `ackNNN` / `rejNNN` acknowledgements** are now recognised (previously only the `CALL:ackNNN` inline form was), so delivery checkmarks are set for ACKs from standard APRS clients such as PinPoint.
- **MeshCom time-sync broadcasts** (`{CET}2026-…`) are kept out of the chat over KISS, matching the ext-udp behaviour (monitor only).
- A **hub client's ACK** is no longer misattributed to WebDesk's own outgoing chat messages when they share a base callsign.
- KISS info fields are decoded as **UTF-8 with a Latin-1 fallback** (Dire Wolf / PinPoint send CP1252).
- A **KISS connect timeout** now shows as a clean "node unreachable" status instead of a raw stack trace.
- **KISS auth detection** no longer stalls on a quiet node that has no authentication configured.
...
- **Native extudp telemetry** – Selected telemetry values (temp, humidity, pressure, temp 2, QNH, gas resistance, CO2) can now be sent directly to the node as a native `"type":"tele"` UDP telegram instead of only as chat text, using the fixed field names the node's firmware expects. Sends happen on value change rather than on a fixed schedule. WebDesk now also checks whether the node's firmware actually applies the values (by watching for its own telemetry echo) and automatically disables the feature if it doesn't, showing the confirmation status in Settings.
...
- IT/ES/FR translations added for the extudp telemetry UI.

### Bugfixes
- Own position/telemetry beacons now shown in the monitor, not just other stations'.
- Duplicate `WeatherRole` assignments in the telemetry mapping table are cleared automatically.
- Extudp telemetry now logs why a send was skipped instead of failing silently.
- `TelemetryExtUdpEnabled` and the extudp slot are now persisted correctly on save.
- Spectrum chart no longer accumulates points from previous scans.
- Own-message callsign and chip contrast (Console Command Helper etc.) fixed in the light theme.

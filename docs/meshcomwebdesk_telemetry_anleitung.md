...
**Wichtig:** Die Werte müssen als JSON-**Zahlen** gesendet werden (`"temp":23.3`),
nicht als Strings (`"temp":"23.3"`) — ArduinoJson auf Node-Seite konvertiert Strings
nicht automatisch, ein String-Wert würde sonst still zu `0.0`.
...
2. Aus MeshComWebDesk: UDP-Paket mit dem `"type":"tele"`-JSON an `<Node-IP>:1799` senden
   ...
   als JSON-Zahlen, nicht als Strings).
...
## 6. WebDesk-seitige Konfiguration (Settings → Telemetrie)

In der Messwert-Zuordnungstabelle hat jede Zeile eine **Rolle**-Spalte. Diese eine
Spalte steuert jetzt zwei Dinge gleichzeitig:

- Das Karten-Popup (Rollen `temp`/`humidity`/`pressure`, wie bisher)
- Das native extudp-Telegramm (alle 7 Rollen: `temp`/`humidity`/`pressure`/`temp2`/
  `qnh`/`gasres`/`co2`) — sofern "Extudp aktiv" eingeschaltet ist

Es gibt keine separate "Slot"-Spalte mehr. Pro Rolle darf nur eine Zeile zugewiesen
sein (mehrfach zugewiesene Rollen: die erste Zeile in der Liste gewinnt, weitere
werden beim Senden ignoriert).

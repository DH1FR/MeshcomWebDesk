...
| **KISS Port** | TCP port of the node's KISS interface (fixed at `8001` by the node), shown only when KISS/TCP is enabled |
...
interface (TCP port 8001), provided the node runs a MeshCom firmware that
...
- **External process announcements** ⚙️ – tick the **⚙️ External** checkbox on an entry to have the beacon text generated by an external script instead of a static message:
  - The script receives a **JSON payload on stdin** and writes the announcement text to **stdout**
  - Same directory and supported file types as bot commands (`BotExternalCommandsPath`, `.py`, `.ps1`, `.bat`, `.exe`)
  - **Empty stdout** → announcement is silently skipped for this slot
  - Configurable **timeout** per entry (default 10 s)
  ...
  **JSON payload written to stdin:**
  ...
  `daysUntil` / `hoursUntil` are `null` when announcing at the exact event time.

- Configured in **Settings → 📅 Termin-Bake / Calendar Beacon**
...
- **External process commands** ⚙️ – tick the **⚙️ External** checkbox in the command table to run an arbitrary executable or script instead of returning a static response:
  ...
  **Example Python script** (ships as `bot-scripts/bot-command-example.py` next to the executable; Docker: inside the `./bot-scripts` volume):
  ```python
  import json, sys
...
└─ Services/
      ...
      ExternalProcessRunner.cs  ← Singleton: launches external scripts from a caller-supplied directory; JSON on stdin → reply from stdout; path-traversal guard, timeout, .exe/.ps1/.bat/.cmd/.py support; shared by bot commands and calendar beacon
      Bot/
...
> **External bot command scripts** are placed in `./bot-scripts/` on the host (mapped to `/app/bot-scripts` in the container).  
> The example script `bot-command-example.py` is included in the image under `/app/bot-scripts/` and is available at startup.  
> To persist your own scripts across container rebuilds, copy them to the host `./bot-scripts/` directory before running `docker compose up -d`.
